...
const getWeather = async ({ city, tokenFromQuery, langFromQuery }) => {
  // Формируем строку `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${OPEN_WEATHER_MAP_API_KEY}&units=metric`;
  const OPEN_WEATHER_MAP_API_KEY = tokenFromQuery || (await getToken());
  const LANGUAGE = langFromQuery || (await getLanguage()) || DEFAULT_LANGUAGE;
...
const getForecast = async ({
  ...
  try {
    ...
    for (const city of cities) {
      const weather = await getWeather({ city, tokenFromQuery, langFromQuery });
